...
def lambda_handler(event, context):
    ...
    global users
    current_user = users[int(event['currentuser'])]
    # if current_user == None: current_user = users[0]
    statuses = make_fake_statuses(event['pagesize'], current_user)
    return {
        'statusCode': 200,
        'body': statuses
    }
...
def make_fake_statuses(pagesize, user):
    num_statuses = int(pagesize)
    result = []
    for i in range(num_statuses // 5):
...
event = {'pagesize':'20', 'nextindex':'0', 'currentuser':'1'}
context = 1
